...
The functions below rely on a set of data files, expressed either as hashes or rang dicts, that are loaded 
lazily to fetch specific property values. These data files themselves are generated by tooling from the 
@hyperlink["https://unicode.org/ucd/"]{Unicode Character Database} (@hyperlink["http://www.unicode.org/Public/UCD/latest/"]{latest files}). The process for generation is described in the separate section @seclink["data-generator"].
